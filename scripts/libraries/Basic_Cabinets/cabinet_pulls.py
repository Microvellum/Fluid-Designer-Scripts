...
class Standard_Pull(fd_types.Assembly):
    ...
    def draw(self):
        g = bpy.context.scene.lm_cabinets
        self.create_assembly()

        self.pull_name = g.pull_name
        
        pull = self.add_object(os.path.join(os.path.dirname(__file__),"Cabinet Pulls",g.pull_category,self.pull_name+".blend"))
